...
export type SearchLeadsResult<T = Record<string, unknown>> = {
  data: T[];
  total: number;
  page: number;
  timestamp: string;
...
function emptySearchResult(page: number): SearchLeadsResult {
  return {
    data: [],
    total: 0,
    page,
...
export async function searchLeads<T = Record<string, unknown>>(
  searchParams: Record<string, unknown> = {},
  page = 1,
): Promise<SearchLeadsResult<T>> {
  try {
    ...
    if (error && typeof error === "object" && "response" in error) {
      if (
        ...
        return emptySearchResult(page) as SearchLeadsResult<T>;
      }
    }
